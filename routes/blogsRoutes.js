...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'public/css/uploads/images'); // Specify the folder for image uploads
    },
...
router.post('/edit-blog/:id', upload.single('imag'), async (req, res) => {
    ...
    const { title, description, contents } = req.body;
    const En = req.body.En === 'true' || req.body.En === 'True'; // Convert En to boolean
    const imageFile = req.file ? `public/css/uploads/images/${req.file.filename}` : null;
    const referer = req.get('Referer') || '/'; // Default to home page if no referrer

    console.log(req.body); // Debug: log incoming data
    console.log('Processed En value:', En); // Debug: check processed En value
    ...
    try {
        ...
        const request = pool.request()
            ...
            .input('Description', sql.NVarChar, description)
            .input('Contents', sql.NText, contents)
            .input('En', sql.Bit, En) // Pass properly converted En
            .input('PostId', sql.Int, id);
